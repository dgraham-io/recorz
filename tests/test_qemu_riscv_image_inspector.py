...
class QemuRiscvImageInspectorTests(unittest.TestCase):
    ...
    def test_inspects_boot_image_summary(self) -> None:
        ...
        expected_object_count = len(seed_objects)
        for section_spec in mvp.DYNAMIC_SEED_OBJECT_SECTION_SPECS:
            expected_object_count += dynamic_sections.seed_layout[section_spec.layout_section_name].count
        ...
        self.assertEqual(summary["seed"]["object_count"], expected_object_count)
        self.assertEqual(summary["seed"]["class_descriptor_count"], len(mvp.CLASS_DESCRIPTOR_KIND_ORDER))
        self.assertEqual(summary["seed"]["class_link_count"], expected_object_count)
        self.assertEqual(summary["seed"]["method_descriptor_count"], len(mvp.METHOD_ENTRY_ORDER))
        self.assertEqual(summary["seed"]["selector_object_count"], len(mvp.SELECTOR_VALUE_ORDER))
        ...
        self.assertEqual(summary["seed"]["global_binding_count"], 14)
        self.assertEqual(summary["seed"]["root_binding_count"], len(mvp.SEED_ROOT_SPECS))
        self.assertEqual(summary["seed"]["globals"]["Transcript"], 0)
        self.assertEqual(summary["seed"]["globals"]["KernelInstaller"], 12)
        self.assertEqual(summary["seed"]["globals"]["Workspace"], 13)
        self.assertEqual(summary["seed"]["globals"]["true"], 14)
        self.assertEqual(summary["seed"]["globals"]["false"], 15)
        self.assertEqual(summary["seed"]["globals"]["TestRunner"], 16)
        self.assertEqual(summary["seed"]["globals"]["Cursor"], 17)
        self.assertEqual(summary["seed"]["globals"]["WorkspaceCursor"], 150)
        self.assertEqual(summary["seed"]["globals"]["WorkspaceSelection"], 151)
        self.assertEqual(summary["seed"]["roots"]["default_form"], 11)
        self.assertEqual(summary["seed"]["roots"]["transcript_behavior"], 148)
        self.assertEqual(summary["seed"]["roots"]["transcript_layout"], 8)
        self.assertEqual(summary["seed"]["roots"]["transcript_style"], 9)
        self.assertEqual(summary["seed"]["roots"]["transcript_metrics"], 147)
        self.assertEqual(summary["seed"]["roots"]["transcript_font"], 149)
