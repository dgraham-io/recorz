...
WIDGET_BOOTSTRAP_PAYLOAD = ROOT / "kernel" / "textui" / "WidgetBootstrap.rz"
...
class QemuRiscv32MakefileTests(unittest.TestCase):
    ...
    def test_run_headless_uses_rv32_framebuffer_target(self) -> None:
        with tempfile.TemporaryDirectory(prefix="qemu-riscv32-makefile-") as temp_dir:
            ...
            self.assertNotIn("-fw_cfg name=opt/recorz-file-in,file=", result.stdout)
            self.assertIn(str(build_dir / "default_file_in_payload.rz"), result.stdout)
            self.assertIn(str(TEXT_BOOTSTRAP_PAYLOAD), result.stdout)
            self.assertIn(str(VIEW_BOOTSTRAP_PAYLOAD), result.stdout)
            self.assertIn(str(WIDGET_BOOTSTRAP_PAYLOAD), result.stdout)
    ...
    def test_dev_loop_reopens_after_snapshot_emission(self) -> None:
        with tempfile.TemporaryDirectory(prefix="qemu-riscv32-makefile-dev-loop-") as temp_dir:
            build_dir = Path(temp_dir)
            snapshot_path = build_dir / "dev" / "live.bin"
