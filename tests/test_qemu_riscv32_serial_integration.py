...
def _normalize_serial_output(output: str) -> str:
...
def _write_development_home_tests_package_file_in(temp_path: Path) -> Path:
    payload_path = temp_path / "development_home_tests_package_file_in.rz"
    payload_path.write_text(
        "\n".join(
            [
                "RecorzKernelPackage: 'Tests' comment: 'Interactive regression fixtures'",
                "!",
                "RecorzKernelClass: #TinySpec superclass: #Object package: 'Tests' instanceVariableNames: ''",
                "!",
                "testPass",
                "    ^true",
                "!",
...
class QemuRiscv32SerialIntegrationTests(unittest.TestCase):
    ...
    def test_workspace_development_home_project_browser_can_open_package_source_and_enter_debugger_on_test_failure(self) -> None:
        with tempfile.TemporaryDirectory(prefix="qemu-riscv32-workspace-development-home-project-tests-") as temp_dir:
            temp_path = Path(temp_dir)
            build_dir = temp_path / "build"
            file_in_payload = _write_development_home_tests_package_file_in(temp_path)
            elf_path = _build_elf(
                build_dir,
            ...
            self.assertIn("INTERACTIVE PACKAGE LIST", output)
            self.assertIn("Tests", output)
            self.assertIn("SOURCE EDITOR", output)
    ...
    def test_workspace_development_home_menu_can_open_the_runtime_metadata_browser_and_return(self) -> None:
        with tempfile.TemporaryDirectory(prefix="qemu-riscv32-workspace-development-home-runtime-metadata-") as temp_dir:
            ...
            try:
                ...
                process.stdin.flush()
                output += _read_until(process, "PRIM", timeout=8.0)
                process.stdin.write("\x0f")
                process.stdin.flush()
                output += _read_until(process, "OPENING MENU", timeout=8.0)
                if process.poll() is None:
            ...
            self.assertIn("RUNTIME METADATA", output)
            self.assertIn("RUN DEV", output)
            self.assertIn("IMG RV64MVP1", output)
    ...
    def test_workspace_browse_packages_interactive_can_edit_a_package_and_return(self) -> None:
        with tempfile.TemporaryDirectory(prefix="qemu-riscv32-workspace-package-home-") as temp_dir:
            ...
            self.assertIn("TextUIRecorzKernelPackage: 'TextUI'", output)
