...
const struct recorz_mvp_seed *recorz_mvp_seed_load(const uint8_t *blob, uint32_t size) {
    ...
    for (object_index = 0U; object_index < global_binding_count; ++object_index) {
        ...
        if (binding_id < RECORZ_MVP_GLOBAL_TRANSCRIPT || binding_id > RECORZ_MVP_GLOBAL_KERNEL_INSTALLER) {
    ...
    for (object_index = RECORZ_MVP_GLOBAL_TRANSCRIPT; object_index <= RECORZ_MVP_GLOBAL_KERNEL_INSTALLER; ++object_index) {
