...
const struct recorz_mvp_seed *recorz_mvp_seed_load(const uint8_t *blob, uint32_t size) {
    ...
    for (global_index = 0U; global_index <= RECORZ_MVP_GLOBAL_FALSE; ++global_index) {
    ...
    for (object_index = 0U; object_index < global_binding_count; ++object_index) {
        ...
        if (binding_id < RECORZ_MVP_GLOBAL_TRANSCRIPT || binding_id > RECORZ_MVP_GLOBAL_FALSE) {
    ...
    for (object_index = RECORZ_MVP_GLOBAL_TRANSCRIPT; object_index <= RECORZ_MVP_GLOBAL_FALSE; ++object_index) {
